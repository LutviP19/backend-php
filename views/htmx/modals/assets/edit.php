...
<div x-show="openModal" 
    ...
    class="p-8">
    <div class="mb-6">
        <h3 class="text-xl font-black text-slate-800">Edit Data Unit</h3>
        <p class="text-xs font-bold text-indigo-500 uppercase tracking-widest hidden md:block"><?= $asset['asset_id'] ?></p>
        <span class="text-xs font-bold text-slate-700"><?= $asset['name'] ?></span>
    </div>

    <form x-ref="form" 
        hx-post="<?= url('/data/asset-update') ?>" 
        hx-include="[name='search'], [name='status_filter'], [name='view_mode']" 
        hx-target="#asset-container" 
        @htmx:before-request="errors = {}" 
        @htmx:response-error="const response = JSON.parse($event.detail.xhr.responseText); errors = response;"
        ...
        <div>
            <label class="text-[10px] font-black text-slate-400 uppercase ml-2">Kode Unit</label>
            <input type="text" name="asset_id" value="<?= $asset['asset_id'] ?>" placeholder="KBT-01" required 
        ...
        <div>
            <label class="text-[10px] font-black text-slate-400 uppercase ml-2">Nama Unit</label>
            <input type="text" name="name" value="<?= $asset['name'] ?>" placeholder="Traktor Kubota" required 
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
            <div x-data="{ 
                    openCat: false, 
                    // Gunakan (int) atau Number agar perbandingan === di JavaScript akurat
                    selectedCatId: <?= (int)$asset['category_id'] ?>, 
                    selectedCatLabel: '<?= htmlspecialchars($asset['category_name']) ?>',
                    categories: [
                ...
                @reset-select.window="selectedCatId = <?= (int)$asset['category_id'] ?>; selectedCatLabel = '<?= htmlspecialchars($asset['category_name']) ?>'">
                
                <label class="text-[10px] font-extrabold text-slate-400 uppercase ml-2 tracking-widest">Kategori Aset</label>
                ...
                <div class="relative mt-1">
                    <button @click="openCat = !openCat" type="button" 
                            ...
                            class="w-full flex items-center justify-between px-5 py-3 bg-slate-50 rounded-2xl text-sm font-semibold text-slate-700 hover:bg-slate-100 transition-all">
                        <span class="flex items-center gap-3">
                            <i x-show="selectedCatId" class="fas text-indigo-500" 
                               :class="categories.find(c => c.id === selectedCatId)?.icon"></i>
                    ...
                    <div x-show="openCat" 
                         ...
                         x-transition:enter-start="opacity-0 scale-95"
                         x-transition:enter-end="opacity-100 scale-100"
                         class="absolute z-[130] w-full mt-2 bg-white border border-slate-100 rounded-2xl shadow-xl p-2 max-h-80 overflow-y-auto no-scrollbar">
                        
                        <template x-for="cat in categories" :key="cat.id">
                            <button type="button" 
                                    @click="selectedCatId = cat.id; selectedCatLabel = cat.label; openCat = false" 
                                    class="w-full flex items-center justify-between px-4 py-3 rounded-xl text-xs font-semibold transition-all"
                                    :class="selectedCatId === cat.id ? 'bg-indigo-50 text-indigo-600' : 'text-slate-600 hover:bg-slate-50'">
                                <span class="flex items-center gap-3">
                                    <div class="w-8 h-8 rounded-lg flex items-center justify-center"
                                         :class="selectedCatId === cat.id ? 'bg-white' : 'bg-slate-50'">
            ...
            <div x-data="{ openStatus: false, selectedStatus: '<?= $asset['status'] ?>', statusLabel: '<?= ucfirst($asset['status']) ?>' }" @reset-select.window="selectedStatus = 'ready'; statusLabel= 'Ready'">
        ...
        <div class="grid grid-cols-2 md:grid-cols-2 gap-4">
            <div x-data="{ open: false, selected: '<?= $asset['icon'] ?>' }" @reset-select.window="selected = 'fa-tractor'">
                ...
                <div class="relative mt-1">
                    <button @click="open = !open" type="button" 
                        ...
                        <span class="flex items-center gap-3">
                            ...
                            <span x-text="selected.replace('fa-', '')" class="line-clamp-1"></span>
                        </span>
            ...
            <div x-data="{ open: false, selected: '<?= $asset['color'] ?>' }" @reset-select.window="selected = 'emerald'">
        ...
        <div x-data="{ health: <?= $asset['health'] ?> }" class="space-y-3">
            <div class="flex justify-between items-center px-2">
                <label class="text-[10px] font-black text-slate-400 uppercase tracking-widest">Health Score</label>
                <span 
                    :class="health < 40 ? 'bg-rose-100 text-rose-600' : (health < 75 ? 'bg-amber-100 text-amber-600' : 'bg-emerald-100 text-emerald-600')"
                    class="px-3 py-1 rounded-full text-xs font-black transition-colors duration-300"
                    x-text="health + '%'"
                ></span>
            </div>

            <div class="relative flex items-center group h-10">
                <div class="absolute w-full h-2 bg-slate-100 rounded-full overflow-hidden">
                    <div 
                        class="h-full transition-all duration-300"
                        :style="`width: ${health}%;`"
                        :class="health < 40 ? 'bg-rose-500' : (health < 75 ? 'bg-amber-500' : 'bg-indigo-500')"
                    ></div>
                </div>
                ...
                <input 
                    ...
                    max="100" 
                    x-model="health"
                    class="absolute w-full h-2 appearance-none bg-transparent cursor-pointer z-10 accent-indigo-600"
                >
            </div>
        ...
        <div class="flex gap-3 pt-4">
            <button type="button" @click="openModal = false; resetForm();" 
                    class="flex-1 py-4 bg-slate-200 text-slate-500 rounded-2xl font-bold text-sm hover:bg-slate-300 transition">
                Batal
            </button>
            <button type="submit" 
                    class="flex-1 py-4 bg-indigo-600 text-white rounded-2xl font-bold text-sm shadow-lg shadow-indigo-100 hover:bg-indigo-700 transition">
                Simpan Perubahan
            </button>
        </div>
    </form>
