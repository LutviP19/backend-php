...
<head>
    ...
    <title>Login - SmartStock AI</title>
...
<body class="bg-slate-50 dark:bg-black flex items-center justify-center min-h-screen p-6 transition-colors duration-500" 
      ...
      @show-toast.window="showToast = true; message = ($event.detail.value || $event.detail); setTimeout(() => showToast = false, 3000)">
    ...
    <div class="glass-effect p-8 md:p-12 rounded-[3rem] shadow-2xl w-full max-w-md border border-slate-200 dark:border-white/10 transition-all duration-500">
        ...
        <form hx-post="<?= url('/login/auth') ?>" 
            ...
            <button type="submit" class="group w-full py-4 bg-indigo-600 text-white rounded-2xl font-bold hover:bg-indigo-700 transition-all shadow-xl shadow-indigo-200 dark:shadow-none flex items-center justify-center gap-3 active:scale-95 disabled:opacity-50">
    ...
    <button @click="isDark = !isDark" class="fixed top-6 right-6 w-10 h-10 rounded-full bg-white dark:bg-slate-900 border border-slate-200 dark:border-white/10 text-slate-500 flex items-center justify-center shadow-md">
        <i class="fas" :class="isDark ? 'fa-sun text-amber-400' : 'fa-moon text-slate-600'"></i>
    </button>
    
    <!-- Audio Web API Handler -->
    <script>
        window.addEventListener("play-error-sound", function() {
            try {
                ...
                console.warn("Audio Context tidak diizinkan sebelum ada interaksi user.", e);
            }
        });
    </script>
</body>
